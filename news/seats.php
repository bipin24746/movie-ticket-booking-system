...
if (isset($_POST['bookSeats'])) {
    ...
    if (isset($_POST['seats']) && is_array($_POST['seats']) && isset($_POST['fname']) && isset($_POST['lname'])) {
        ...
        if (count($availableSeats) > 0) {
            ...
            try {
                foreach ($availableSeats as $seatId) {
                    // Insert the booking details along with the user's name (first name and last name)
                    $sqlInsertBooking = "INSERT INTO booking (movie_id, show_date, show_time, seat_id, total_price, fname, lname) 
                                        VALUES ($movieId, '$selectedDate', '$showTime', $seatId, $totalPrice, '$fname', '$lname')";
