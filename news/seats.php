                ...
                if (count($availableSeats) > 0) {
                    ...
                    try {
                        foreach ($availableSeats as $seatId) {
                            // Insert the booking details along with the user's ID
                            $sqlInsertBooking = "INSERT INTO booking (movie_id, show_date, show_time, seat_id, total_price, uid) 
                                                VALUES ($movieId, '$selectedDate', '$showTime', $seatId, $totalPrice, '$uid')";
