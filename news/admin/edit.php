  ...
  if ($result && $result->num_rows > 0) {
    ...
    $actor = $_POST['actor'];
      ...
      if ($_FILES["image"]["name"]) {
        ...
        if (in_array($file_ext, $allowed_extensions)) {
          if ($file_size < 5242880) { // Max file size: 5MB (you can adjust this value)
            ...
            if (move_uploaded_file($file_tmp, $destination)) {
              ...
              $sql = "UPDATE movie SET name='$name', genre = '$genre', industry = '$industry', language = '$language', duration = '$duration', reldate = '$reldate', director = '$director', actor = '$actor', price='$price', fdate='$fdate', sdate = '$sdate', fshow = '$fshow', sshow = '$sshow', tshow = '$tshow',image='$image' WHERE id=$id";
              $result = $conn->query($sql);
        ...
        $sql = "UPDATE movie SET name='$name', genre = '$genre', industry = '$industry', language = '$language', duration = '$duration', reldate = '$reldate', director = '$director', actor = '$actor', price='$price', fdate='$fdate', sdate = '$sdate', fshow='$fshow' WHERE id=$id";
        $result = $conn->query($sql);
...
<body>
  ...
  <input type="date" id="name" name="sdate" value="<?php echo $sdate; ?>"><br> -->
      ...
      <input type="time" name="sshow" id="time" value="<?php echo $sshow; ?>" required><br><br>

      <label for="time">Third Show</label>
      <input type="time" name="tshow" id="time" value="<?php echo $tshow; ?>" required><br><br> -->
